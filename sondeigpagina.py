...
# =============================================================================
# === 0. ESTILS I CONFIGURACIÓ GENERAL ======================================
# =============================================================================

def load_css():
    """Injecta CSS personalitzat per millorar la UI/UX de l'aplicació."""
    st.markdown("""
    <style>
        /* === Estils Globals === */
        .stApp {
            background-color: #F0F2F5; /* Un gris molt suau per al fons */
        }
        
        /* === Estils Pantalla de Benvinguda === */
        .welcome-container {
            display: grid;
            place-items: center;
            height: 100vh;
        ...
        .lightning-svg {
            position: absolute; opacity: 0; animation: flash 0.6s ease-out;
        }
        @keyframes flash {
...
# =========================================================================
# === 2. FUNCIONS DE CÀLCUL I ANÀLISI (AMB CACHING) =======================
# =========================================================================
...
def generate_public_warning(p_levels, t_profile, td_profile, wind_speed, wind_dir):
    ...
    warnings_config = {
        ...
        "PLUGES INTENSES": ("🌊", "AVÍS PER PLUGES INTENSES", "Risc de pluges persistents i fortes. Possible acumulació d'aigua.", "darkblue"),
        "PLUJA MODERADA": ("🌧️", "AVÍS PER PLUJA MODERADA", "Cel cobert amb pluja contínua i moderada.", "steelblue"),
        "PLUJA FEBLE": ("💧", "PREVISIÓ DE PLUJA FEBLE", "S'esperen plugims o ruixats febles i intermitents.", "cadetblue"),
        "TORNADO": ("🌪️", "AVÍS PER TORNADO", "Condicions favorables per a tornados. Vigileu el cel i seguiu alertes.", "darkred"),
        "BASE ALTA": ("💨", "AVÍS PER TEMPESTES DE BASE ALTA", "Risc de ratxes de vent fortes i sobtades (downbursts).", "darkorange"),
        "PEDRA": ("🟣", "AVÍS PER PEDRA", "Tempestes violentes amb risc de pedra grossa. Protegiu vehicles.", "purple"),
        "TEMPESTES": ("⚡", "AVÍS PER TEMPESTES", "Tempestes fortes amb llamp, pluja intensa i possible calamarsa.", "darkorange"),
        "SENSE AVISOS": ("✅", "SENSE AVISOS", "Condicions meteorològiques sense riscos significatius.", "green"),
    }

    if fz_h < 1500 or sfc_temp.m < 5:
        if sfc_temp.m <= 0.5:
            return warnings_config["NEU"]
        else:
            p_low = p_levels[p_levels > (p_levels[0].m - 300) * units.hPa]
            if np.any(t_profile[:len(p_low)].m > 0.5) and sfc_temp.m < 2.5:
                return warnings_config["GEBRADORA"]
    try:
        ...
        if np.sum(layer_mask) > 2:
            ...
            if rh_mean_layer > 0.85 and cape.magnitude < 350:
                if pwat_layer.m > 25: return warnings_config["PLUGES INTENSES"]
                elif pwat_layer.m > 15: return warnings_config["PLUJA MODERADA"]
                else: return warnings_config["PLUJA FEBLE"]
    except Exception:
        pass
    if cape.m >= 1000:
        shear_0_6, s_0_1, srh_0_1, srh_0_3 = calculate_storm_parameters(p_levels, wind_speed, wind_dir)
        if srh_0_1 > 150 and shear_0_6 > 15: return warnings_config["TORNADO"]
        if lfc_h > 3000: return warnings_config["BASE ALTA"]
        if cape.m > 2000: return warnings_config["PEDRA"]
        return warnings_config["TEMPESTES"]
    
    return warnings_config["SENSE AVISOS"]
...
def _draw_cumulus_mediocris(ax, base_km, top_km):
    ...
    widths = base_width + np.random.uniform(-0.1, 0.1, len(altitudes)); widths[0] = max(widths[0], 0.3)
    ...
    for _ in range(num_particles):
        y_progress = random.betavariate(2, 2); y = base_km + y_progress * cloud_height
...
def _draw_cumulus_castellanus(ax, base_km, top_km):
    ...
    patches_base = [Ellipse((random.uniform(-1.7, 1.7), base_km + (random.random() ** 2) * base_thickness), width=random.uniform(0.7, 1.6), height=random.uniform(0.1, 0.25), facecolor=(b,b,b), alpha=random.uniform(0.1, 0.3), lw=0) for _ in range(120) for b in [random.uniform(0.8, 0.9)]]
    ...
    for i in range(num_turrets):
        ...
        turret_height = turret_top_y - turret_base_y; max_width = random.uniform(0.25, 0.4)
        patches_turret = []
        for _ in range(random.randint(60, 90)):
            y = turret_base_y + (random.random() ** 0.8) * turret_height
            normalized_y = (y - turret_base_y) / turret_height
            current_width = max_width * np.sin(np.pi * normalized_y)
            x = turret_center_x + random.uniform(-current_width * 0.9, current_width * 0.9)
            size = random.uniform(0.1, 0.3) * (1 + normalized_y * 0.5)
            brightness = 0.75 + (0.98 - 0.75) * (normalized_y ** 0.8)
...
def _draw_stratiform_cotton_clouds(ax, base_km, top_km):
    patches = [Ellipse((random.uniform(-1.7, 1.7), random.uniform(base_km, top_km)), random.uniform(0.4, 0.9), random.uniform(0.15, 0.3), facecolor=(b, b, b), alpha=random.uniform(0.3, 0.6), lw=0) for _ in range(200) for b in [random.uniform(0.88, 0.98)]]
    ax.add_collection(PatchCollection(patches, match_original=True, zorder=9))
...
def _draw_precipitation(ax, precip_base_km, ground_km, p_type, center_x=0.0, sub_cloud_rh=0.4):
    if p_type == 'virga':
        alpha = np.clip(sub_cloud_rh * 0.6, 0.15, 0.55); fall_percentage = sub_cloud_rh / 0.5
        fall_distance = (precip_base_km - ground_km) * fall_percentage
        end_y = max(precip_base_km - fall_distance, ground_km + (0.3 if sub_cloud_rh < 0.5 else 0))
        ...
        ax.add_patch(Rectangle((center_x - 0.8, ground_km), 1.6, precip_base_km - ground_km, facecolor='cornflowerblue', alpha=0.35, lw=0, zorder=5))
    elif p_type == 'hail':
...
def _draw_saturation_layers(ax, p_levels, t_profile, td_profile):
    try:
        ...
        while i < len(saturated_indices):
            ...
            patches=[Ellipse((random.uniform(-1.5,1.5), random.uniform(h_bottom,h_top)), random.uniform(0.3,0.8), random.uniform(0.05,0.1)*(1+h_top-h_bottom), facecolor=(b,)*3, alpha=random.uniform(0.1,0.5), lw=0) for _ in range(int(100+300*(h_top-h_bottom))) for b in [random.uniform(0.65,0.85)]]
...
def create_skewt_figure(_p, _t, _td, _ws, _wd): # Underscore to avoid conflict with outer scope
    ...
    td_profile = np.minimum(_t, _td)
    skew.plot(_p, _t, 'r', linewidth=2, label='Temperatura (T)')
    skew.plot(_p, td_profile, 'b', linewidth=2, label='Punt de Rosada (Td)')
    parcel_prof = mpcalc.parcel_profile(_p, _t[0], _td[0]).to('degC')
    skew.plot(_p, parcel_prof, 'k--', linewidth=2, label='Bombolla Adiabàtica')
    wb_profile = mpcalc.wet_bulb_temperature(_p, _t, _td)
    skew.plot(_p, wb_profile, color='purple', linewidth=1.5, label='Tª Bombolla Humida')
    skew.shade_cape(_p, _t, parcel_prof, facecolor='yellow', alpha=0.3)
    skew.shade_cin(_p, _t, parcel_prof, facecolor='black', alpha=0.3)
    _, _, lcl_p, _, lfc_p, _, el_p, _, _ = calculate_thermo_parameters(_p, _t, _td)
    if lcl_p: ax.plot(ax.get_xlim(), [lcl_p.m, lcl_p.m], 'gray', linestyle='--', label='LCL')
    if lfc_p: ax.plot(ax.get_xlim(), [lfc_p.m, lfc_p.m], 'purple', linestyle='--', label='LFC')
    if el_p: ax.plot(ax.get_xlim(), [el_p.m, el_p.m], 'red', linestyle='--', label='EL')
...
def create_cloud_drawing_figure(_p, _t, _td, convergence_active, precipitation_type, lfc_h, cape, base_km, top_km, cloud_type):
    ...
    _draw_saturation_layers(ax, _p, _t, _td)
    if base_km is not None and top_km is not None:
        ...
        elif cloud_type in ["Cumulonimbus (Multicèl·lula)", "Supercèl·lula"]: _draw_cumulonimbus(ax, base_km, top_km)
    ...
    elif not np.any((_t.m - _td.m) <= 1.5): _draw_clear_sky(ax)
    if precipitation_type and base_km is not None:
        ...
        try:
            ...
            sub_cloud_mask = (_p >= p_base_precip) & (_p <= _p[0])
            if np.any(sub_cloud_mask):
                rh_profile = mpcalc.relative_humidity_from_dewpoint(_t, _td)
...
def create_cloud_structure_figure(_p, _t, _td, _ws, _wd, convergence_active):
    ...
    cape, *_ = calculate_thermo_parameters(_p, _t, _td)
    base_km, top_km = _calculate_dynamic_cloud_heights(_p, _t, _td, convergence_active)
    ...
    try:
        u, v = mpcalc.wind_components(_ws, _wd); h_km = mpcalc.pressure_to_height_std(_p).to('km').m
        ...
        altitudes = np.linspace(visual_base_km, top_km, num=50); u_at_alts = f_u(altitudes)
        horizontal_offsets = u_at_alts * 0.02
        shear_0_6, s_0_1, srh_0_1, srh_0_3 = calculate_storm_parameters(_p, _ws, _wd)
        ...
        _, _, lcl_p, lcl_h, _, _, _, _, _ = calculate_thermo_parameters(_p, _t, _td)
        feature = None
...
def create_radar_figure(_p, _t, _td, _ws, _wd):
    ...
    cape, *_ = calculate_thermo_parameters(_p, _t, _td)
    try:
        heights_amsl = mpcalc.pressure_to_height_std(_p).to('m'); heights_agl = (heights_amsl - heights_amsl[0]).to('km')
        layer_mask = (heights_agl.m >= 0) & (heights_agl.m <= 4)
        if np.sum(layer_mask) > 2:
            rh_layer = mpcalc.relative_humidity_from_dewpoint(_t[layer_mask], _td[layer_mask])
            pwat_layer = mpcalc.precipitable_water(_p[layer_mask], _td[layer_mask]).to('mm')
            rh_mean_layer = np.mean(rh_layer)
            if rh_mean_layer > 0.85 and cape.magnitude < 350:
                x, y = np.meshgrid(np.linspace(-50, 50, 100), np.linspace(-50, 50, 100))
                max_dbz = np.clip(15 + pwat_layer.m, 15, 45)
                Z = max_dbz + gaussian_filter(np.random.randn(100, 100), sigma=8) * (max_dbz * 0.2)
                Z = np.clip(Z, 0, 50)
    ...
    if cape.m < 100:
        ax.text(0, 0, "Sense precipitació significativa", ha='center', va='center', color='white', fontsize=9); return fig
    shear_0_6, *_ = calculate_storm_parameters(_p, _ws, _wd)
    _, _, _, _, lfc_p, _, el_p, _, _ = calculate_thermo_parameters(_p, _t, _td)
    mean_u, mean_v = (0,0) * units('m/s')
    if lfc_p and el_p:
        p_mask = (_p >= el_p) & (_p <= lfc_p)
        if np.sum(p_mask) > 1:
            u, v = mpcalc.wind_components(_ws[p_mask], _wd[p_mask])
    ...
    xx, yy = np.meshgrid(np.linspace(-50, 50, 150), np.linspace(-50, 50, 150))
...
# =========================================================================
# === 4. ESTRUCTURA DE L'APP I LÒGICA DE VISUALITZACIÓ ====================
# =========================================================================
...
def show_welcome_screen_animated():
    ...
    if random.random() < 0.35:
        top, left, size, rot, color = random.randint(0, 50), random.randint(5, 95), random.randint(150, 400), random.randint(-25, 25), random.choice(['#FFFFFF', '#f0f8ff'])
        svg_path = "M13 1L1 14H9L7 23L23 10H15L13 1Z"
        style = f"top:{top}%;left:{left}%;width:{size}px;height:{size}px;transform:rotate({rot}deg) translateX(-50%);"
        lightning_html = f'<div class="lightning-layer"><div class="lightning-svg" style="{style}"><svg viewBox="0 0 24 24" fill="{color}"><path d="{svg_path}"></path></svg></div></div>'
    ...
    with st.container():
        ...
        with col1:
            ...
            st.write("Analitza dades reals per hora.")
        ...
        with col2:
            ...
            st.write("Modifica paràmetres i experimenta.")
...
def display_parameters_tab(params):
    st.subheader("Paràmetres Termodinàmics i de Cisallament")
    
    st.markdown("##### Energia")
    cols = st.columns(4)
    cols[0].metric("CAPE", f"{params['cape'].m:.0f} J/kg")
    cols[1].metric("CIN", f"{params['cin'].m:.0f} J/kg")
    
    st.markdown("##### Nivells Clau")
    cols = st.columns(4)
    cols[0].metric("0°C (FZL)", f"{params['fz_h']/1000:.2f} km")
    cols[1].metric("LCL (Base núvol)", f"{params['lcl_p'].m:.0f} hPa" if params['lcl_p'] else "N/A")
    cols[2].metric("LFC (Convecció)", f"{params['lfc_p'].m:.0f} hPa" if params['lfc_p'] else "N/A")
    cols[3].metric("EL (Top convectiu)", f"{params['el_p'].m:.0f} hPa" if params['el_p'] else "N/A")

    st.markdown("##### Cisallament i Helicitat (SRH)")
    cols = st.columns(4)
    cols[0].metric("Shear 0-6km", f"{params['shear_0_6']:.1f} m/s")
    cols[1].metric("SRH 0-1km", f"{params['srh_0_1']:.1f} m²/s²")
    cols[2].metric("SRH 0-3km", f"{params['srh_0_3']:.1f} m²/s²")
    
    st.markdown("##### Humitat")
    cols = st.columns(4)
    cols[0].metric("PWAT Total", f"{params['pwat_total'].m:.1f} mm")
    cols[1].metric("PWAT 0-4km", f"{params['pwat_0_4'].m:.1f} mm")
    rh_display = "N/A"
    try: rh_display = f"{params['rh_0_4'].m*100:.0f}%" if hasattr(params['rh_0_4'], 'm') else f"{params['rh_0_4']*100:.0f}%"
    except: pass
...
def run_display_logic(p, t, td, ws, wd, obs_time):
    ...
    storm_params = calculate_storm_parameters(p, ws, wd)
    shear_0_6, s_0_1, srh_0_1, srh_0_3 = storm_params
    ...
    try:
        heights_agl = (mpcalc.pressure_to_height_std(p) - mpcalc.pressure_to_height_std(p[0])).to('km')
        layer_mask = (heights_agl.m >= 0) & (heights_agl.m <= 4)
        if np.sum(layer_mask) > 2:
...
# =========================================================================
# === 5. MODES DE FUNCIONAMENT DE L'APP ===================================
# =========================================================================

def run_live_mode():
    with st.sidebar:
        st.pyplot(create_logo_figure())
        st.header("Navegació (Mode Viu)")
        if st.button("⬅️ Tornar a l'inici", use_container_width=True):
            st.session_state.app_mode = 'welcome'; st.rerun()
        st.toggle("Activar convergència (Forçar convecció)", value=st.session_state.get('convergence_active', True), key='convergence_active', help="Simula una forçant dinàmica que ajuda a iniciar la convecció.")
    ...
    if 'live_initialized' not in st.session_state:
        ...
        now = datetime.now(); hour_12 = now.hour % 12 or 12; am_pm = 'am' if now.hour < 12 else 'pm'
        ...
        st.session_state.loaded_sounding_index = -1; st.session_state.live_initialized = True
    ...
    with st.sidebar:
        st.selectbox("Selecciona una hora:", options=st.session_state.existing_files, index=st.session_state.sounding_index, key='selectbox_widget', on_change=lambda: setattr(st.session_state, 'sounding_index', st.session_state.existing_files.index(st.session_state.selectbox_widget)))

    main_cols = st.columns([1, 10, 1])
    if main_cols[0].button('←', use_container_width=True, disabled=(st.session_state.sounding_index == 0)):
        st.session_state.sounding_index -= 1; st.rerun()
    if main_cols[2].button('→', use_container_width=True, disabled=(st.session_state.sounding_index >= len(st.session_state.existing_files) - 1)):
        st.session_state.sounding_index += 1; st.rerun()
...
def run_sandbox_mode():
    with st.sidebar:
        st.pyplot(create_logo_figure())
        st.header("Configuració (Laboratori)")
        if st.button("⬅️ Tornar a l'inici", use_container_width=True):
            st.session_state.app_mode = 'welcome'; st.rerun()
        st.toggle("Activar convergència (Forçar convecció)", value=st.session_state.get('convergence_active', True), key='convergence_active', help="Simula una forçant dinàmica que ajuda a iniciar la convecció.")
    ...
    if 'sandbox_initialized' not in st.session_state:
        ...
        if not soundings:
            st.error("No s'ha trobat 'sondeigproves.txt'. Aquest mode no pot funcionar."); return
        st.session_state.sandbox_original_data = soundings[0]
        st.session_state.sandbox_p_levels = soundings[0]['p_levels'].copy()
        st.session_state.sandbox_t_profile = soundings[0]['t_initial'].copy()
        st.session_state.sandbox_td_profile = soundings[0]['td_initial'].copy()
        st.session_state.sandbox_ws = soundings[0]['wind_speed_kmh'].to('m/s')
        st.session_state.sandbox_wd = soundings[0]['wind_dir_deg'].copy()
    ...
    with st.sidebar:
        if st.button("🔄 Reiniciar al perfil original", use_container_width=True):
            ...
            st.session_state.sandbox_t_profile = data['t_initial'].copy(); st.session_state.sandbox_td_profile = data['td_initial'].copy()
            st.session_state.sandbox_ws = data['wind_speed_kmh'].to('m/s'); st.session_state.sandbox_wd = data['wind_dir_deg'].copy()
            st.rerun()
        st.markdown("---")
        st.subheader("Modificació Manual")
        new_sfc_t = st.slider("🌡️ Temp. Superfície (°C)", -20.0, 50.0, st.session_state.sandbox_t_profile[0].magnitude, 0.5)
        new_sfc_td = st.slider("💧 Rosada Superfície (°C)", -20.0, new_sfc_t, st.session_state.sandbox_td_profile[0].magnitude, 0.5)
        st.session_state.sandbox_t_profile[0] = new_sfc_t * units.degC; st.session_state.sandbox_td_profile[0] = new_sfc_td * units.degC
        st.markdown("---")
...
if __name__ == '__main__':
    st.set_page_config(layout="wide", page_title="Visor de Sondejos de Tempestes.cat")
