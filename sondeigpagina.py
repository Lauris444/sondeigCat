...
def parse_all_soundings(filepath):
    ...
    NOU: Tradueix automàticament el text de l'hora del francès al català.
    """
    all_soundings_data = []
    ...
    def process_sounding_block(block_lines):
        ...
        # NOU: Diccionari per a paraules generals
        general_fr_to_ca = {
            'Run': 'Model',
            'locale': 'local',
            'du': 'del'
        ...
        for line in block_lines:
            ...
            try:
                ...
                print(f"Advertència: Error processant línia '{line_strip}'. Error: {e}")
                continue
        
        if not p_list or len(p_list) < 2: return None
        
        # Bucle per traduir les línies d'hora trobades
        translated_lines = []
        for line in time_lines:
            translated_line = line
            # Traduir dies i mesos primer
            ...
            # Traduir paraules generals (Run, locale, etc.)
            for fr_word, ca_word in general_fr_to_ca.items():
                # Usem regex per assegurar que només es reemplacen paraules senceres i sense distinció de majúscules/minúscules
    ...
    # Processar totes les línies del fitxer
    for line in lines:
...
class AdvancedSkewT:
    def __init__(self, p_levels, t_initial, td_initial, wind_speed_kmh=None, wind_dir_deg=None, observation_time="Hora no disponible"):
        ...
        self.nav_text_ax = self.fig.add_axes([0.01, 0.01, 0.2, 0.05])
        self.nav_text_ax.axis('off')
        self.nav_text_obj = self.nav_text_ax.text(0, 0.5, "", fontsize=12, weight='bold', va='center')
        
        # Nou eix per a l'hora
    ...
    def setup_radar_sim(self):
        ...
        # S'han eliminat les etiquetes dels eixos
    ...
    def setup_plot(self):
        ...
        self.line_t, = self.skew.plot([], [], 'r', linewidth=2, picker=5, label='Temperatura (T)')
        self.line_td, = self.skew.plot([], [], 'b', linewidth=2, picker=5, label='Punt de Rosada (Td)')
        ...
        # S'ha eliminat el text de pressió de l'esquerra

    def setup_surface_controls(self):
        ax_surface_bg = self.fig.add_axes([0.77, 0.03, 0.18, 0.06])
        ax_surface_bg.set_facecolor('lightgray'); ax_surface_bg.axis('off')
        ax_surface_bg.text(0.5, 0.75, "Ajusta Pressió (hPa)", ha='center', va='center', fontsize=9)
        ax_surface_input = self.fig.add_axes([0.78, 0.04, 0.09, 0.03])
        self.surface_input = TextBox(ax_surface_input, "", initial=str(int(self.current_surface_pressure.magnitude)))
        ax_surface_button = self.fig.add_axes([0.88, 0.04, 0.06, 0.03])
        self.surface_button = Button(ax_surface_button, "OK", color='lightblue')
        self.surface_button.on_clicked(self.change_surface_pressure)
    ...
    def setup_extra_buttons(self):
        ax_conv_button = self.fig.add_axes([0.77, 0.53, 0.22, 0.04])
        self.convergence_button = Button(ax_conv_button, "Convergència (ON)", color='gold', hovercolor='gold')
        self.convergence_button.on_clicked(self.on_convergence_click)

    def on_convergence_click(self, event):
        self.convergence_active = not self.convergence_active
        if self.convergence_active:
            self.convergence_button.label.set_text("Convergència (ON)"); self.convergence_button.color = 'gold'
        else:
            self.convergence_button.label.set_text("Convergència (OFF)"); self.convergence_button.color = 'lightgoldenrodyellow'
        self.update_plot()
    ...
    def on_click(self, event):
        if self.text_box is not None and event.inaxes != self.text_box.ax:
            self.text_box.ax.remove(); self.text_box = None; self.fig.canvas.draw_idle()
    ...
    def change_surface_pressure(self, event):
        try:
            new_p = float(self.surface_input.text) * units.hPa
            if self.original_p_levels[-1].m < new_p.m <= self.original_p_levels[0].m:
                ...
                self.update_plot()
                self.update_ground_patch()
        except ValueError as e: print(f"Error en el valor de pressió: {str(e)}")
    ...
    def adjust_profiles_to_new_surface(self):
        try:
            ...
            print(f"Error ajustant els perfils: {str(e)}"); self.reset_profiles()
    ...
    def calculate_storm_parameters(self):
        try:
            ...
            print(f"Error calculant paràmetres de tempesta: {e}"); return 0.0, 0.0, 0.0, 0.0
    ...
    def _draw_cumulonimbus(self, ax, base_km, top_km):
        # --- REALISTIC DRAWING V3: WITH DEDICATED ANVIL LOGIC ---
        updraft_center_x = 0
        num_points = 20
        ...
        anvil_center_y = (anvil_base_alt + top_km) / 2
    ...
    def _draw_cumulus_mediocris(self, ax, base_km, top_km, num_particles=200):
        ...
        for _ in range(num_particles):
            ...
            brightness = np.clip(raw_brightness, 0.0, 1.0)
            puff_color = (brightness, brightness, brightness)
            ax.add_patch(Circle((x, y), size, 
                                facecolor=puff_color, 
    ...
    def _draw_cumulus_fractus(self, ax, base_km, thickness, num=150):
        ...
        for _ in range(num):
            ...
            patches.append(Ellipse((random.gauss(0,0.5),y),random.uniform(0.2,0.4),
                             random.uniform(0.3,0.7)*random.uniform(0.2,0.4),
                             angle=random.uniform(-25,25), facecolor=self._get_cloud_color(y,base_km,base_km+thickness,b_min=0.6,b_max=0.8),alpha=0.5,lw=0))
    ...
    def _draw_saturation_layers(self, ax):
        try:
            ...
            while i < len(saturated_indices):
                ...
                i=j+1
        except Exception as e: print(f"No s'ha pogut dibuixar la capa d'estrats: {e}")
    ...
    def draw_clouds(self):
        ...
        if real_base_km and real_top_km:
            ...
            visual_top_km = visual_base_km+(real_top_km-real_base_km)
    ...
    def _draw_base_feature(self, ax, f_type, base_x_left, base_x_right, base_y, ground_y):
        ...
        if f_type == 'lowering':
            ...
            ], facecolor='dimgray', edgecolor='gray', zorder=z))

        elif f_type == 'wall_cloud':
            top_width_factor = 0.75; bottom_width_factor = 0.55
            wall_height = 0.35
            ...
            for i in range(3):
                offset_x = random.uniform(-0.05, 0.05); offset_y = random.uniform(0.0, 0.05)
                alpha = random.uniform(0.1, 0.25); color = random.choice(['#404040', '#505050'])
            ...
            for _ in range(25):
                ...
                ax.add_patch(Ellipse((fx, fy), random.uniform(0.02, 0.08), random.uniform(0.02, 0.05),
                                     angle=random.uniform(0, 360), facecolor='#3a2d1c', alpha=random.uniform(0.5, 0.8), zorder=z + 2))
    ...
    def draw_cloud_structure(self):
        ...
        if not base_km or not top_km or cape.m < 100:
            self.ax_cloud_structure.text(0.5, 0.5, "Sense Estructura Convectiva", ha='center', va='center',
                                       transform=self.ax_cloud_structure.transAxes, fontsize=9, color='white',
        ...
        try:
            ...
            # Augment de la densitat de les barbes de vent
            barb_heights = np.arange(0, min(20, h_km.max()), 1)
            barb_u = f_u(barb_heights) * units('m/s')
            barb_v = f_v(barb_heights) * units('m/s')
            
            self.ax_shear_barbs.barbs(np.zeros_like(barb_heights), barb_heights, 
                                    barb_u.to('knots').m, barb_v.to('knots').m, 
            ...
            base_l_pt = l_pts[0]
            base_r_pt = (updraft_widths[0] + horizontal_offsets[0], altitudes[0])
            ...
            if feature:
                base_x_left_coord = l_pts[0][0]; base_x_right_coord = r_pts[0][0]
                self._draw_base_feature(self.ax_cloud_structure, feature, base_x_left_coord, base_x_right_coord, visual_base_km, self.ground_height_km)
            
            self.ax_cloud_label.text(0.5, 0.5, base_label, ha='center', va='center', fontsize=10, weight='bold', color=l_color,
                                     bbox=dict(facecolor=bg_color, alpha=0.8, edgecolor='black', boxstyle='round, pad=0.4'))

        except Exception as e:
            print(f"Error drawing cloud structure: {e}")
    ...
    def update_plot(self):
        try:
            ...
            # Les barbes de vent del Skew-T han estat eliminades
            if self.wind_barbs:
            ...
            self.fig.canvas.draw_idle()
        except Exception as e: print(f"Error fatal actualitzant el gràfic: {str(e)}")

    def on_pick(self, event):
        if event.mouseevent.button != 1: return
        self.selected_line = event.artist
        self.selected_point_idx = np.abs(self.selected_line.get_ydata().magnitude - event.mouseevent.ydata).argmin()
        if self.text_box is not None: self.text_box.ax.remove()
        axbox = self.fig.add_axes([0.45, 0.05, 0.1, 0.03]) 
        profile = self.t_profile if self.selected_line == self.line_t else self.td_profile
        self.text_box = TextBox(axbox, 'Temp:', initial=f"{profile[self.selected_point_idx].magnitude:.1f}")
        self.text_box.on_submit(self.update_temperature)

    def update_temperature(self, text):
        if self.selected_point_idx is None: return
        try:
            profile = self.t_profile if self.selected_line == self.line_t else self.td_profile
            profile[self.selected_point_idx] = float(text) * units.degC
            self.update_plot()
        except ValueError: pass
        finally:
            if self.text_box: self.text_box.ax.remove(); self.text_box = None
            
    def load_new_data(self, sounding_data):
        """Carrega noves dades de sondeig, incloent l'hora."""
        print("\n--- ACTUALITZANT DADES ---")
        
        # 1. Càrrega de dades bàsiques
        ...
        # 3. Processament del vent
        try:
            if 'wind_speed_kmh' in sounding_data and sounding_data['wind_speed_kmh'] is not None:
                ws_units = sounding_data['wind_speed_kmh'].units
                self.original_wind_speed = sounding_data['wind_speed_kmh'].to('m/s') if ws_units == units('kph') else sounding_data['wind_speed_kmh'].copy()
            else:
            ...
            print(f"Error actualitzant vent: {e}")
            self.original_wind_speed = np.zeros_like(self.original_p_levels.magnitude) * units('m/s')
            self.original_wind_dir = np.zeros_like(self.original_p_levels.magnitude) * units.degrees
            self.original_u = self.original_wind_speed.copy()
            self.original_v = self.original_wind_speed.copy()
        
        # 4. Reiniciar perfils i forçar actualització
        self.reset_profiles()
        self._force_wind_update()
        self.update_plot()

    def _force_wind_update(self):
        """Actualització forçada de les dades de vent"""
        if not hasattr(self, 'original_u') or self.original_u is None:
            self.original_u, self.original_v = mpcalc.wind_components(
                self.original_wind_speed,
                self.original_wind_dir
            )
...
if __name__ == '__main__':
    base_files = [
        "sondeig.txt", "sondeig1.txt", "sondeig2.txt",
        "sondeig4.txt", "sondeig5.txt"
    ...
    for file in base_files:
        if file in existing_files:
            print(f"Arxiu trobat: {file}")
        else:
            print(f"Arxiu NO trobat: {file} (serà ignorat)")
    
    if not existing_files:
        print("\nError: No s'ha trobat cap arxiu de sondeig!")
        print("Assegura't que tens almenys un d'aquests arxius al directori:")
        print("\n".join(base_files))
        exit()
    
    current_index = 0
    
    def load_sounding(filepath):
        try:
            print(f"\nIntentant carregar: {filepath}")
            soundings = parse_all_soundings(filepath)
            if soundings:
                print(f"S'han trobat {len(soundings)} sondejos a {filepath}. Utilitzant el primer.")
                return soundings[0]
            else:
                print(f"Avís: L'arxiu {filepath} no conté dades de sondeig vàlides.")
                return None
        except Exception as e:
            print(f"Error crític carregant {filepath}: {str(e)}")
            return None
    
    print(f"\nCarregant sondeig inicial: {existing_files[current_index]}")
    initial_data = load_sounding(existing_files[current_index])
    
    if not initial_data:
        print("Error: No s'han pogut carregar dades inicials. S'atura el programa.")
        exit()
    
    skew_instance = AdvancedSkewT(**initial_data)
    
    def update_nav_text():
        current_file = existing_files[current_index]
        text = f"Arxiu: {current_file} ({current_index + 1}/{len(existing_files)})"
        skew_instance.nav_text_obj.set_text(text)
        skew_instance.fig.canvas.draw_idle()
        print(f"\nVisualitzant: {current_file}")
    
    def go_next(event):
        global current_index
        current_index = (current_index + 1) % len(existing_files)
        print(f"\nNavegant al següent arxiu: {existing_files[current_index]}")
        new_data = load_sounding(existing_files[current_index])
        if new_data:
            skew_instance.load_new_data(new_data)
            update_nav_text()
        else:
            print("No s'han pogut carregar les dades, navegació cancel·lada.")
            current_index = (current_index - 1) % len(existing_files)

    def go_prev(event):
        global current_index
        current_index = (current_index - 1 + len(existing_files)) % len(existing_files)
        print(f"\nNavegant a l'arxiu anterior: {existing_files[current_index]}")
        new_data = load_sounding(existing_files[current_index])
        if new_data:
            skew_instance.load_new_data(new_data)
            update_nav_text()
    ...
    ax_prev = plt.axes([0.25, 0.03, 0.1, 0.04])
    btn_prev = Button(ax_prev, '← Anterior', color='lightgoldenrodyellow', hovercolor='yellow')
    btn_prev.on_clicked(go_prev)
    
    ax_next = plt.axes([0.65, 0.03, 0.1, 0.04])
    btn_next = Button(ax_next, 'Següent →', color='lightgoldenrodyellow', hovercolor='yellow')
    btn_next.on_clicked(go_next)
    
    print("\nPreparat per visualitzar sondejos.")
    print("Controls:")
    print("- Botó 'Anterior' / 'Següent' per canviar d'arxiu.")
    print("- Fes clic a les línies de T/Td per editar-les.")
