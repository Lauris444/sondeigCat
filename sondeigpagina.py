...
def create_skewt_figure(p, t, td, ws, wd):
    ...
    td_min = np.minimum(t, td)
    skew.plot(p, t, 'r', linewidth=2, label='Temperatura (T)'); skew.plot(p, td_min, 'b', linewidth=2, label='Punt de Rosada (Td)')
    ...
    _, _, lcl_p, _, lfc_p, _, el_p, _, _ = calculate_thermo_parameters(p, t, td)
    xlims = ax.get_xlim()
...
def create_cloud_drawing_figure(p, t, td, conv, precip_type, lfc_h, cape, base_km, top_km, cloud_type):
...
def create_cloud_structure_figure(p, t, td, ws, wd, conv_active):
    ...
    cape, *_, lcl_h = calculate_thermo_parameters(p, t, td)
    ...
    try:
        ...
        f_u, f_v = interp1d(unique_h, u.m[idx], fill_value='extrapolate'), interp1d(unique_h, v.m[idx], fill_value='extrapolate')
        ...
        alts = np.linspace(visual_base_km, top_km, 50); offsets = f_u(alts) * 0.02
        ...
        if (top_km - visual_base_km) > 4.0:
            ...
            if len(anvil_idx) > 0:
                ...
                max_stretch = abs(u_anvil_top) * 0.06 * np.sign(u_anvil_top if u_anvil_top != 0 else 1)
...
def display_whatsapp_chat(full_conversation, logo_b64):
    st.markdown(f"""<style>
        .chat-container {{ background-color: #E5DDD5; padding: 15px; border-radius: 10px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif; max-height: 500px; overflow-y: auto; display: flex; flex-direction: column; gap: 12px; border: 1px solid #ccc; }}
    ...
    def handle_send_message():
        st.session_state.chat_progress += 1
        ...
        with chat_placeholder, st.container(border=True):
            with st.container(height=350):
                ...
                for i, (speaker, message) in enumerate(visible_log):
                    ...
                    if is_user:
                        st.markdown(f'<div class="message-row {align_class}"><div class="message {msg_class} {sent_class}">{message}<span class="time">12:34 PM <span class="ticks">✔✔</span></span></div></div>', unsafe_allow_html=True)
                    else:
                        st.markdown(f'<div class="message-row {align_class}"><img src="data:image/png;base64,{logo_b64}" class="profile-pic"><div class="message {msg_class}">{message}<span class="time">12:34 PM</span></div></div>', unsafe_allow_html=True)
            ...
            if next_user_message_index < len(full_conversation):
                cols = st.columns([8,2])
                cols[0].text_input("Missatge:", full_conversation[next_user_message_index][1], disabled=True, label_visibility="collapsed")
...
def main():
    ...
    with tab1:
        ...
        if not st.session_state.get('chat_open', False):
            ...
            with cols[1]:
                if st.button("Obrir Anàlisi de Tempestes.cat", key="open_chat_btn", use_container_width=True, type="primary"):
                    st.session_state.chat_open = True; st.rerun()
        else: 
            display_whatsapp_chat(full_conversation, logo_base64)
