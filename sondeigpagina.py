...
class AdvancedSkewT:
    ...
    # La majoria de mètodes interns de la classe romanen igual.
    # S'ometen per brevetat (reset_profiles, setup_radar_sim, càlculs, dibuixos, etc.)
    # ... (Copiar aquí tots els mètodes interns de la classe `AdvancedSkewT` de la resposta anterior) ...
    # És important que tots els mètodes de càlcul i dibuix estiguin presents.
    ...
    def draw_static_radar_echo(self):
        ...
        x = np.linspace(-50, 50, 150); y = np.linspace(-50, 50, 150); xx, yy = np.meshgrid(x, y)
        x_rot = xx * np.cos(angle_rad) + yy * np.sin(angle_rad); y_rot = -xx * np.sin(angle_rad) + yy * np.cos(angle_rad)
    ...
    def setup_plot(self):
        ...
        self.skew.plot_dry_adiabats(alpha=0.3, color='orange'); self.skew.plot_moist_adiabats(alpha=0.3, color='green')
    ...
    def change_surface_pressure(self, new_p_val):
        try:
            ...
            if self.original_p_levels[-1].m < new_p.m <= self.original_p_levels[0].m:
                ...
                self.ground_height_km = mpcalc.pressure_to_height_std(self.current_surface_pressure).to('km').magnitude
                self.adjust_profiles_to_new_surface(); self.update_plot(); self.update_ground_patch()
            else: st.warning(f"La pressió ha d'estar entre {self.original_p_levels[-1].m:.0f} i {self.original_p_levels[0].m:.0f} hPa.")
        except (ValueError, TypeError) as e: st.error(f"Error en el valor de pressió: {str(e)}")
    ...
    def calculate_thermo_parameters(self):
        try:
            ...
            try:
                ...
                p_range = np.arange(p.m.min(), p.m.max()); t_range = t_interp(p_range)
    ...
    def calculate_storm_parameters(self):
        try:
            ...
            u_i = np.interp(h_interp.m, h_u.m, u_u.m) * units('m/s'); v_i = np.interp(h_interp.m, h_u.m, v_u.m) * units('m/s')
            p_i = np.interp(h_interp.m, h_u.m, p_u.m) * units.hPa
            u_6, v_6 = mpcalc.bulk_shear(p_i, u_i, v_i, height=h_interp, depth=6000 * units.meter); s_0_6 = mpcalc.wind_speed(u_6, v_6).m
            u_1, v_1 = mpcalc.bulk_shear(p_i, u_i, v_i, height=h_interp, depth=1000 * units.meter); s_0_1 = mpcalc.wind_speed(u_1, v_1).m
    ...
    def draw_parameters_box(self):
        ...
        lfc_t = f"{lfc_p.m:>6.0f} hPa" if lfc_p else "   N/A "; el_t = f"{el_p.m:>6.0f} hPa" if el_p else "   N/A "; lcl_t = f"{lcl_p.m:>6.0f} hPa" if lcl_p else "   N/A "
    ...
    def _draw_cumulonimbus(self, ax, base_km, top_km):
        updraft_center_x = 0; altitudes = np.linspace(base_km, top_km, 20); anvil_base_alt = top_km * 0.8
        tower_indices = np.where(altitudes < anvil_base_alt)[0]
        if len(tower_indices) == 0: tower_indices = np.arange(len(altitudes))
        tower_alts = altitudes[tower_indices]; widths = 0.5 * (1 + 0.8 * np.sin(np.pi * (tower_alts - base_km) / (top_km - base_km))) + np.random.uniform(-0.05, 0.05, len(tower_indices))
        r_pts = [(updraft_center_x + widths[i], tower_alts[i]) for i in range(len(tower_indices))]; l_pts = [(updraft_center_x - widths[i], tower_alts[i]) for i in range(len(tower_indices))]
        main_poly_pts = [(l_pts[0][0], l_pts[0][1])] + r_pts + l_pts[::-1]; ax.add_patch(Polygon(main_poly_pts, facecolor='#d8d8d8', lw=0, zorder=10))
        for _ in range(120):
            idx = random.randint(1, len(tower_alts) - 1); y = tower_alts[idx] + random.uniform(-0.3, 0.3)
            max_x_at_y = np.interp(y, tower_alts, widths, left=widths[0], right=widths[-1]); x = updraft_center_x + random.uniform(-max_x_at_y, max_x_at_y)
            size = random.uniform(0.2, 0.6) * (1 + (y - base_km) / (top_km - base_km)); brightness = np.clip(0.85 + 0.15 * ((y - base_km) / (top_km - base_km)), 0.0, 1.0)
        ...
        for _ in range(80):
            y = random.uniform(anvil_base_alt, top_km); height_factor = 1 + (y - anvil_base_alt) / (top_km - anvil_base_alt)
            x = updraft_center_x + random.uniform(-anvil_spread * height_factor, anvil_spread * height_factor); width = random.uniform(0.5, 1.2) * height_factor; height = random.uniform(0.05, 0.15)
            brightness = random.uniform(0.95, 1.0); ax.add_patch(Ellipse((x, y), width, height, facecolor=(brightness,)*3, alpha=random.uniform(0.1, 0.3), lw=0, zorder=12))

    def _draw_cumulus_mediocris(self, ax, base_km, top_km, num_particles=200):
        center_x = 0; altitudes = np.linspace(base_km, top_km, 15); widths = 0.3 * (1 + np.sin(np.pi * (altitudes - base_km) / (top_km - base_km + 0.01))) + np.random.uniform(-0.05, 0.05, 15)
        r_pts = [(center_x + widths[i], altitudes[i]) for i in range(15)]; l_pts = [(center_x - widths[i], altitudes[i]) for i in range(15)]
        main_poly_pts = [(l_pts[0][0], l_pts[0][1])] + r_pts + l_pts[::-1]; ax.add_patch(Polygon(main_poly_pts, facecolor='#e0e0e0', lw=0, zorder=10))
        for _ in range(num_particles):
            idx = random.randint(1, 14); y = altitudes[idx] + random.uniform(-0.2, 0.2); max_x_at_y = np.interp(y, altitudes, widths, left=widths[0], right=widths[-1])
            x = center_x + random.uniform(-max_x_at_y, max_x_at_y); size = random.uniform(0.1, 0.4); brightness = np.clip(0.8 + 0.2 * ((y - base_km) / (top_km - base_km)), 0.0, 1.0)
    ...
    def _draw_precipitation(self, ax, base_km, ground_km, p_type, center_x=0.0):
        if p_type == 'virga':
            ...
            for _ in range(50): xs = center_x + random.uniform(-0.6, 0.6); xe = xs + random.uniform(-0.1, 0.1); ax.plot([xs,xe],[base_km*0.95,end_y],color='lightblue',alpha=random.uniform(0.1,0.3),lw=1.2,zorder=5)
        elif p_type in ['rain', 'sleet']: 
            width = 1.6; ax.add_patch(Rectangle((center_x-width/2,ground_km),width,base_km-ground_km,facecolor='cornflowerblue',alpha=0.25,lw=0,zorder=5))
            for _ in range(100): x = center_x+random.uniform(-width/2,width/2); ax.plot([x,x],[base_km*0.95,ground_km],color='blue',alpha=random.uniform(0.1,0.3),lw=0.8,zorder=6)
    ...
    def draw_clouds(self):
        ...
        if real_base_km and real_top_km:
            visual_base_km = max(real_base_km, self.ground_height_km+0.5); cloud_depth = real_top_km-real_base_km; visual_top_km = visual_base_km + cloud_depth
    ...
    def _draw_base_feature(self, ax, f_type, base_x_left, base_x_right, base_y, ground_y):
        ...
        if f_type == 'lowering': ax.add_patch(Polygon([(base_x_left, base_y), (base_x_right, base_y), (base_x_right * 0.9 + center_x * 0.1, base_y - 0.2), (base_x_left * 0.9 + center_x * 0.1, base_y - 0.2)], facecolor='dimgray', edgecolor='gray', zorder=z))
            ...
            top_w=0.75; bot_w=0.55; wall_h=0.35; top_l=center_x-(width*top_w/2); top_r=center_x+(width*top_w/2); bot_l=center_x-(width*bot_w/2); bot_r=center_x+(width*bot_w/2)
            ax.add_patch(Polygon([(top_l, base_y), (top_r, base_y), (bot_r, base_y - wall_h), (bot_l, base_y - wall_h)], facecolor='#383838', edgecolor='#202020', lw=0.5, zorder=z))
            for i in range(3):
                off_x=random.uniform(-0.05,0.05); off_y=random.uniform(0.0,0.05); alpha=random.uniform(0.1,0.25); color=random.choice(['#404040','#505050'])
                s_top_l=top_l+(bot_l-top_l)*0.1*i+off_x; s_top_r=top_r+(bot_r-top_r)*0.1*i+off_x; s_bot_l=bot_l-(bot_l-top_l)*0.2*i+off_x; s_bot_r=bot_r-(bot_r-top_r)*0.2*i+off_x
                ax.add_patch(Polygon([(s_top_l, base_y-off_y), (s_top_r, base_y-off_y), (s_bot_r, base_y-wall_h+off_y), (s_bot_l, base_y-wall_h+off_y)], facecolor=color, alpha=alpha, zorder=z+1, lw=0))
            ...
            for _ in range(25):
                angle, r = random.uniform(0, 2*np.pi), random.uniform(0.7*0.4, 0.7*0.9); fx, fy = center_x+r*np.cos(angle), ground_y+random.uniform(0,0.3)
    ...
    def draw_cloud_structure(self):
        ...
        self.ax_cloud_label.axis('off'); self.ax_cloud_structure.set_title("Estructura Vertical i Cisallament", fontsize=10)
        self.ax_cloud_structure.set_facecolor('skyblue'); self.ax_cloud_structure.add_patch(Rectangle((-1.5, 0), 3, self.ground_height_km, color='darkgreen', alpha=0.7, zorder=1, hatch='//'))
        self.ax_cloud_structure.set(ylim=(0, 20), xlim=(-1.5, 1.5), ylabel="Altitud (km)", xticks=[]); self.ax_cloud_structure.grid(True, linestyle='--', alpha=0.3)
        ...
        if not base_km or not top_km or cape.m < 100:
            self.ax_cloud_structure.text(0.5, 0.5, "Sense Estructura Convectiva", ha='center', va='center', transform=self.ax_cloud_structure.transAxes, fontsize=9, color='white', bbox=dict(facecolor='darkblue', alpha=0.7)); self.ax_shear_barbs.axis('off'); return
        visual_base_km = max(base_km, self.ground_height_km + 0.5)
        try:
            u, v = mpcalc.wind_components(self.wind_speed, self.wind_dir); h_km = mpcalc.pressure_to_height_std(self.p_levels).to('km').m
            unique_h, idx = np.unique(h_km, return_index=True)
            if len(unique_h) < 2: return
            f_u=interp1d(unique_h, u.m[idx], bounds_error=False, fill_value="extrapolate"); f_v=interp1d(unique_h, v.m[idx], bounds_error=False, fill_value="extrapolate")
            barb_h=np.arange(0, min(20, h_km.max()), 1); barb_u=f_u(barb_h)*units('m/s'); barb_v=f_v(barb_h)*units('m/s')
            self.ax_shear_barbs.barbs(np.zeros_like(barb_h), barb_h, barb_u.to('knots').m, barb_v.to('knots').m, length=7, pivot='middle', color='k')
            alts = np.linspace(visual_base_km, top_km, num=50); u_alts = f_u(alts); h_offsets = u_alts * 0.02; shear_0_6, *_ = self.calculate_storm_parameters()
            shear_factor = np.clip(shear_0_6 / 35, 0.4, 2.5); widths = 0.4 * (1 + 0.5 * np.sin(np.pi * (alts - visual_base_km) / (top_km - visual_base_km + 0.01))) * shear_factor
            anvil_ext = np.zeros_like(alts); u_anvil_top = 0
            if (top_km-visual_base_km) > 4.0:
                anvil_base_alt = top_km * 0.80; anvil_indices = np.where(alts >= anvil_base_alt)[0]
                if len(anvil_indices) > 0:
                    u_anvil_top = f_u(top_km); wind_dir = np.sign(u_anvil_top) if u_anvil_top!=0 else 1
                    max_stretch = abs(u_anvil_top)*0.06; growth = (alts[anvil_indices]-anvil_base_alt)/(top_km-anvil_base_alt)
                    anvil_ext[anvil_indices] = max_stretch * wind_dir * growth**1.5
            r_pts = [(widths[i] + h_offsets[i] + anvil_ext[i], alts[i]) for i in range(len(alts))]; l_pts = [(-widths[i] + h_offsets[i], alts[i]) for i in range(len(alts))]
            self.ax_cloud_structure.add_patch(Polygon(r_pts + l_pts[::-1], facecolor='white', edgecolor='lightgray', alpha=0.95, zorder=10))
            if (top_km-visual_base_km) > 3.0 and u_anvil_top!=0:
                p_start_alt=top_km*0.85; an_ext_start=np.interp(p_start_alt,alts,anvil_ext); upd_off_start=np.interp(p_start_alt,alts,h_offsets)
                p_center_x=upd_off_start+an_ext_start*0.75; p_width=widths[np.argmin(np.abs(alts-p_start_alt))]; p_alts=np.linspace(p_start_alt, self.ground_height_km, 20)
                u_p_alts=f_u(p_alts); init_off=u_p_alts[0]*0.01; p_offsets=u_p_alts*0.01-init_off
                r_p_pts=[(p_center_x+p_width/2+p_offsets[i], p_alts[i]) for i in range(len(p_alts))]; l_p_pts=[(p_center_x-p_width/2+p_offsets[i], p_alts[i]) for i in range(len(p_alts))]
                self.ax_cloud_structure.add_patch(Polygon(r_p_pts+l_p_pts[::-1], facecolor='cornflowerblue', alpha=0.3, lw=0, zorder=8))
            base_l=l_pts[0]; base_r=(widths[0]+h_offsets[0],alts[0]); self.ax_cloud_structure.add_patch(Polygon([base_l, base_r, (base_r[0],base_r[1]-0.1), (base_l[0],base_l[1]-0.1)], facecolor='dimgray', edgecolor='gray', alpha=0.9, zorder=11))
            cape, _, _, lcl_h, _, _, _, _, _ = self.calculate_thermo_parameters(); shear_0_6, shear_0_1, srh_0_3, srh_0_1 = self.calculate_storm_parameters()
            label, l_col, bg_col, feature = "Base Plana", 'black', 'lightgray', None
            if top_km-base_km>4.0 and cape.m>500:
                if (srh_0_1>=150 and lcl_h<=1000 and shear_0_1>=15): label,bg_col,l_col,feature="TORNADO POSSIBLE",'darkred','white','tornado'
                elif (srh_0_1>100 and lcl_h<1200 and shear_0_1>12): label,bg_col,l_col,feature="Núvol d'Embut/Tuba",'red','white','funnel'
                elif srh_0_3>150 and shear_0_6>18 and cape.m>1000: label,bg_col,l_col,feature="Núvol Paret (Wall Cloud)",'orange','white','wall_cloud'
                elif shear_0_1>8 and lcl_h<1500: label,bg_col,l_col,feature="Base Rebaixada (Lowering)",'gold','black','lowering'
            if feature: self._draw_base_feature(self.ax_cloud_structure, feature, l_pts[0][0], r_pts[0][0], visual_base_km, self.ground_height_km)
            self.ax_cloud_label.text(0.5,0.5,label,ha='center',va='center',fontsize=10,weight='bold',color=l_col, bbox=dict(facecolor=bg_col,alpha=0.8,edgecolor='black',boxstyle='round,pad=0.4'))
        except Exception: pass
    ...
    def generate_public_warning(self):
        ...
        if fz_h < 1500 or sfc_temp.m < 5:
            if sfc_temp.m <= 0.5: return "AVÍS PER NEU", "Es preveu nevada a cotes baixes amb acumulacions significatives. Preveu problemes de circulació i temperatures baixes.", "navy"
                ...
                if np.any(self.t_profile[:len(p_low)].m > 0.5) and sfc_temp.m < 2.5: return "AVÍS PER PLUJA GEBRADORA", "Pluja gelant o aiguaneu que pot crear glaçades. Extremi les precaucions.", "dodgerblue"
            ...
            if srh_0_1 > 150 and shear_0_1 > 15: return "AVÍS PER TORNADO", "Condicions favorables per a la formació de tornados. Vigileu el cel i estigueu atents a alertes.", "darkred"
    ...
    def update_plot(self):
        try:
            ...
            # La generació i mostra del text del xat ara es fa a la funció main de Streamlit
    ...
    def load_new_data(self, sounding_data):
        ...
        try:
            if 'wind_speed_kmh' in sounding_data and sounding_data['wind_speed_kmh'] is not None: self.original_wind_speed = sounding_data['wind_speed_kmh'].to('m/s')
            else: self.original_wind_speed = np.zeros_like(self.original_p_levels.magnitude) * units('m/s')
            if 'wind_dir_deg' in sounding_data and sounding_data['wind_dir_deg'] is not None: self.original_wind_dir = sounding_data['wind_dir_deg'].copy()
            else: self.original_wind_dir = np.zeros_like(self.original_p_levels.magnitude) * units.degrees
            self.original_u, self.original_v = mpcalc.wind_components(self.original_wind_speed, self.original_wind_dir)
        except Exception as e:
            st.warning(f"Error processant dades de vent: {e}"); self.original_wind_speed = np.zeros_like(self.original_p_levels.magnitude)*units('m/s'); self.original_wind_dir = np.zeros_like(self.original_p_levels.magnitude)*units.degrees
    ...
    def _force_wind_update(self):
        ...
        if np.count_nonzero(valid_orig_mask) < 2:
             self.u = np.full_like(self.p_levels.magnitude, np.nan) * units('m/s'); self.v = np.full_like(self.p_levels.magnitude, np.nan) * units('m/s')
        else:
            ...
            interp_u = interp1d(unique_p, u_orig_valid[idx], bounds_error=False, fill_value="extrapolate"); interp_v = interp1d(unique_p, v_orig_valid[idx], bounds_error=False, fill_value="extrapolate")
            self.u = interp_u(self.p_levels.magnitude) * units('m/s'); self.v = interp_v(self.p_levels.magnitude) * units('m/s')
