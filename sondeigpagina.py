...
# Imports afegits per al web scraping
import requests
...
def create_city_mountain_scape():
    ...
    star_x = np.random.uniform(0, 100, 200); star_y = np.random.uniform(15, 60, 200)
    star_s = np.random.uniform(0.5, 2.5, 200); star_alpha = np.random.uniform(0.5, 1, 200)
    ...
    for x_base in np.arange(0, 70, 0.5):
        ...
        color = (random.uniform(0.05, 0.1),)*3
        building = Rectangle((x_base, 0), building_width, building_height, facecolor=color, zorder=10)
...
def fetch_sounding_from_meteociel(url):
    ...
    Aquesta versió utilitza headers més realistes per intentar evitar bloquejos.
    """
    try:
        # Aquestes capçaleres imiten un navegador Chrome real a Windows
        headers = {
        ...
        # Utilitzem 'html.parser' que és estàndard i no requereix instal·lacions addicionals
        ...
        if pre_tag:
            ...
            # FALLADA (amb depuració)
            st.error("No s'ha pogut trobar la taula de dades (<pre>) a la pàgina de Meteociel.")
            st.warning("El servidor ha respost, però no amb les dades esperades. Això pot ser una mesura anti-scraping o un canvi a la web.")
            page_title = soup.find('title').get_text() if soup.find('title') else "Títol no trobat"
            st.code(f"Títol de la pàgina rebuda: {page_title}")
            st.code(response.text[:2000], language='html') # Mostrem una part més gran de l'HTML
            return None
